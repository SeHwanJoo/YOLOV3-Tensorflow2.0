...
from core.yolov3 import YOLOv3, decode, YOLOv3_tiny


def get_bbox_data(image_name):
    txt = []
    with open(cfg.TEST.ANNO_PATH + image_name + ".txt", 'r') as f:
        while 1:
            line = f.readline()
            if not line: break
            txt.append(list(map(float, line.split())))
    return np.array(txt, dtype=int)
...
model.load_weights("./checkpoints/yolov3-tiny0")
# utils.load_weights(model, "./yolo-voc.weights")
