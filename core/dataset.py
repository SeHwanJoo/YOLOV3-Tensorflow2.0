...
class Dataset(object):
    ...
    def load_bbox_txt(self, filename):
        with open(self.anno_path + filename + ".txt", 'r') as f:
    ...
    def load_annotations(self, dataset_type):
        ...
        with open(self.annot_path, 'r') as f:
            for line in f:
                line = line.replace('\n', '')
                annotations.append(line + ' ' + self.load_bbox_txt(line.split('/')[-1][:-4]))
        if dataset_type == 'test':
            annotations = annotations[0:len(annotations)//10]
